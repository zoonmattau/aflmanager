...
import { VENUES, VENUE_NAME_TO_ID, venueHasRoof } from '@/data/venues'
...
import { simulateQuarterChain } from '@/engine/match/chainEngine'
...
export interface MatchContext {
  ...
  // Rotation plan events — applied during possession loop
  homeRotationEvents: RotationEvent[]
  awayRotationEvents: RotationEvent[]
  allAwayPlayers: Player[]  // full squad (field + bench) for away rotation purposes
  ...
  // Running time per quarter (minutes of play, excl. stoppages) — ~18–22 each
  quarterRunningMinutes: number[]

  /** Ticks generated by chain engine for each quarter (only populated for live matches). */
  quarterTicks: import('@/types/matchTick').MatchTick[][]
}
...
function generateWeatherModifiers(
  ...
  // Roofed venues can never produce wet or windy conditions
  if (hasRoof) {
    baseWeights.clear = Math.max(0.05, baseWeights.clear + baseWeights.windy + baseWeights.wet)
    baseWeights.windy = 0
    baseWeights.wet   = 0
  }
  ...
  // Roofed venues always have a firm surface
  if (hasRoof && (groundCondition === 'muddy' || groundCondition === 'heavy')) {
    return {
      condition,
...
export function previewMatchWeather(
  ...
  return generateWeatherModifiers(new SeededRNG(seed), venueId, matchDay, month, venueHasRoof(venueId, venueRoofOverrides, venueName, customStadiums))
}
...
/**
 * Generate a realistic AI rotation schedule for one team.
 * Produces ~3 rotation events per quarter, cycling bench players through the
 * field at fixed-ish times so TOG% stays in a realistic 65–85% range.
 */
function generateAIRotationSchedule(
  rng: SeededRNG,
  starters: Player[],
  allPlayers: Player[],
  quartersPerMatch: number,
): RotationEvent[] {
  const bench = allPlayers.filter((p) => !starters.some((s) => s.id === p.id))
  if (bench.length === 0) return []

  const events: RotationEvent[] = []
  const onField = new Set(starters.map((p) => p.id))
  // Bench pool — ordered list we cycle through
  const benchPool = bench.map((p) => p.id)
  let evtId = 0

  for (let q = 1; q <= quartersPerMatch; q++) {
    // Rotate at roughly minutes 7, 14, 21 — small jitter per game
    const times = [
      6 + rng.nextInt(0, 3),
      13 + rng.nextInt(0, 3),
      20 + rng.nextInt(0, 2),
    ...
    for (const minute of times) {
      if (benchPool.length === 0) continue
      // Rotate 1 player (occasionally 2 in non-first quarters)
      const swaps = q > 1 && rng.chance(0.35) ? 2 : 1
      for (let s = 0; s < swaps; s++) {
        if (benchPool.length === 0) break
        const fieldArr = [...onField]
        const offId = fieldArr[rng.nextInt(0, fieldArr.length - 1)]
        const benchIdx = rng.nextInt(0, benchPool.length - 1)
        const onId = benchPool[benchIdx]
        events.push({
          id: `ai-${++evtId}`,
        ...
        onField.delete(offId)
        onField.add(onId)
        benchPool.splice(benchIdx, 1)
        benchPool.push(offId)
      }
    }
  }
  return events
...
export function createMatchContext(input: SimulateMatchInput): MatchContext {
  ...
  const roofed = venueHasRoof(venueId, input.venueRoofOverrides, venue, input.customStadiums)
  const weather = generateWeatherModifiers(rng, venueId, input.matchDay, input.month, roofed)
  const matchWeatherData = generateMatchWeather(rng, venueId, roofed)
  ...
  return {
    ...
    allAwayPlayers: awayAvailablePlayers,
    homeSubstitute,
    awaySubstitute,
    homeStats,
    awayStats,
    ...
    positionFitMults,
    homeRotationEvents: input.homeRotationPlan?.length
      ? input.homeRotationPlan.filter((e) => e.playerOffId && e.playerOnId)
      : generateAIRotationSchedule(rng, homePlayers, homeAvailablePlayers, quartersPerMatch),
    awayRotationEvents: generateAIRotationSchedule(rng, awayPlayers, awayAvailablePlayers, quartersPerMatch),
    midMatchAdjustments: [],
    ...
    matchWeatherData,
    quarterRunningMinutes: Array.from({ length: quartersPerMatch }, () => rng.nextInt(18, 23)),
    quarterTicks: [],
  }
}
...
function pbpAbbrev(p: { firstName: string; lastName: string; jerseyNumber?: number }): string {
...
export function simulateQuarter(ctx: MatchContext, quarterIndex: number): void {
  ...
  for (let p = 0; p < possessions; p++) {
    ...
    // Apply any due home rotation events before this possession
    ...
    // Apply any due away rotation events before this possession
    for (const event of pendingAwayRotations) {
      if (appliedRotationIds.has(event.id)) continue
      if (currentMinute < event.minute) break

      const offIdx = ctx.awayActivePlayers.findIndex((pl) => pl.id === event.playerOffId)
      const onPlayer = ctx.allAwayPlayers.find((pl) => pl.id === event.playerOnId)

      if (
        offIdx !== -1 &&
        onPlayer &&
        !ctx.awayActivePlayers.some((pl) => pl.id === event.playerOnId)
      ) {
        ctx.awayActivePlayers.splice(offIdx, 1, onPlayer)
        if (!ctx.awayStats.some((s) => s.playerId === event.playerOnId)) {
          ctx.awayStats.push(createEmptyPlayerStats(event.playerOnId))
        }
        appliedRotationIds.add(event.id)
      } else {
        appliedRotationIds.add(event.id)
...
export function finalizeMatch(ctx: MatchContext, skipUserSubForSide?: 'home' | 'away'): Match {
  ...
  for (const [teamStats, teamPlayers, teamRisk] of [
    ...
    for (const stat of teamStats) {
      ...
      const totalRunningMinutes = ctx.quarterRunningMinutes.reduce((s, m) => s + m, 0)
      const rawClock = stat.minutesPlayed > 0
        ? stat.minutesPlayed + Math.round(extraLoad * rng.nextInt(1, 3))
        : estimateMinutesPlayed(stat) + Math.round(extraLoad * rng.nextInt(3, 6))
      // Convert clock-time estimate (0–120 basis) to running-time minutes
      stat.minutesPlayed = Math.min(totalRunningMinutes, Math.round(rawClock * totalRunningMinutes / 120))
  ...
  const result: MatchResult = {
    homeScores,
    awayScores,
    quarterRunningMinutes: ctx.quarterRunningMinutes,
    homeTotalScore,
    awayTotalScore,
...
/**
 * Simulate a single quarter using the unified chain engine.
 * Returns MatchTick[] directly — each disposal in the sim IS a display tick.
 * The chain engine handles zone progression, scoring, stat accumulation, and commentary.
 * Used for the live match path (user watching) only.
 */
export function simulateQuarterLive(ctx: MatchContext, quarterIndex: number): import('@/types/matchTick').MatchTick[] {
  const ticks = simulateQuarterChain(ctx, quarterIndex)
  ctx.quarterTicks.push(ticks)
  return ticks
}
...
function applySubstituteStrategy(params: {
  ...
  // Sub gets 16–42 clock-minutes; we'll rescale to running-time in finalizeMatch
